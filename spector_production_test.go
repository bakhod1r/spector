...
// x-spector-source (no file paths, no line numbers) and the console's
...
func TestGenerateProductionStripsSource(t *testing.T) {
	...
	if strings.Contains(string(blob), "x-spector-source") {
		t.Error("Production JSON still contains x-spector-source")
	}
}

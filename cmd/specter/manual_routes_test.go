...
func TestConfigRoutesSupplementDocument(t *testing.T) {
	dir := writeTree(t, map[string]string{"main.go": dynSrc, "specter.yaml": routesConfigYAML})
	...
	var doc struct {
		Paths map[string]map[string]struct {
			...
			Manual    bool            `json:"x-specter-manual"`
			Responses map[string]any  `json:"responses"`
			Tags      []string        `json:"tags"`
			Source    json.RawMessage `json:"x-specter-source"`
		} `json:"paths"`
	}
	...
	if !op.Manual {
		t.Error("supplemented route not marked x-specter-manual")
	}

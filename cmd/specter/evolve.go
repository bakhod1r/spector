...
func runEvolve(cfg specter.Config, ec evolveConfig, stdout, stderr io.Writer) int {
	fail := func(err error) int {
		fmt.Fprintln(stderr, "specter:", err)
		return 1
	}

	changes, err := specter.Evolve(cfg, specter.EvolveOptions{
		SinceRev:     ec.since,
	...
	if summary.Breaking > 0 {
		fmt.Fprintf(stderr, "specter: %d breaking change(s) found\n", summary.Breaking)
	}

...
func TestAllWriteFileFailureExits1(t *testing.T) {
	...
	if !strings.Contains(stderr, "spector:") {
...
// A spector.yaml next to the source is applied the same way spector.json is,
// with identical keys — including the nested camelCase bearerFormat.
func TestYAMLConfigFillsDocument(t *testing.T) {
	dir := writeTree(t, map[string]string{"main.go": ginSrc, "spector.yaml": yamlConfigSrc})
...
// JSON is tried first, so a spector.json wins over a spector.yaml sitting beside
// it — today's behaviour is never silently overridden.
func TestJSONPrecedenceOverYAML(t *testing.T) {
	yaml := "title: FromYAML\nversion: \"9.9\"\n"
	dir := writeTree(t, map[string]string{"main.go": ginSrc, "spector.json": configSrc, "spector.yaml": yaml})
	code, stdout, stderr := exec(t, "-dir", dir)
	if code != 0 {
		t.Fatalf("exit = %d, stderr: %s", code, stderr)
	}
	if strings.Contains(stdout, "FromYAML") {
		t.Errorf("spector.yaml overrode spector.json; JSON should win:\n%s", stdout)
	}
}
...
func TestProdFlagHidesSource(t *testing.T) {
	...
	if !strings.Contains(stdout, "x-spector-source") {
		t.Fatalf("dev document has no x-spector-source to hide:\n%s", stdout)
	}
	...
	if strings.Contains(stdout, "x-spector-source") {
		t.Errorf("-prod document still contains x-spector-source:\n%s", stdout)
	}
}
...
func TestMergeLearnedFillsAndEnriches(t *testing.T) {
  ...
  "components":{"schemas":{}}
	...
	if !strings.Contains(stdout, "x-spector-observed") {
		t.Errorf("observed-only route is not marked x-spector-observed:\n%s", stdout)
	}

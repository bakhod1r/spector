...
func TestStdoutWriteFailureExits1(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "spector:") {
...
const reexecEnv = "SPECTOR_TEST_RUN_MAIN"
...
func TestConfigFileFillsTheDocument(t *testing.T) {
	dir := writeTree(t, map[string]string{"main.go": ginSrc, "spector.json": configSrc})
	code, stdout, stderr := exec(t, "-dir", dir, "-config", filepath.Join(dir, "spector.json"))
	if code != 0 {
		t.Fatalf("exit = %d, stderr: %s", code, stderr)
	}
...
func TestFlagsBeatTheConfigFile(t *testing.T) {
	dir := writeTree(t, map[string]string{"main.go": ginSrc, "spector.json": configSrc})
	code, stdout, stderr := exec(t, "-dir", dir, "-config", filepath.Join(dir, "spector.json"), "-version", "9.9")
	if code != 0 {
		t.Fatalf("exit = %d, stderr: %s", code, stderr)
	}
	var doc struct {
...
// A spector.json sitting in the scanned directory is picked up without being
...
func TestConfigFileFoundInScanDir(t *testing.T) {
	...
	if !strings.Contains(stdout, "https://api.example.com") {
		t.Errorf("servers missing; the adjacent spector.json was not read:\n%s", stdout)
	}
}

...
// A spector.json next to the scanned source is applied as a default, same as
...
func TestMCPConfigFileApplies(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "spector.json"), []byte(cfgJSON), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestMCPExplicitMissingConfigIsToolError(t *testing.T) {
	res, err := handleGenerateOpenAPI(context.Background(), callReq(map[string]any{
		"dir":    "../../examples/ginapp",
		"config": "/nonexistent/spector.json",
	}))
	if err != nil {
		t.Fatal(err)
	}

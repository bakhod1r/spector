...
// runMCP serves spector's generators and linter as MCP tools over stdio, so an
...
func newMCPServer() *server.MCPServer {
	s := server.NewMCPServer("spector", "0.1.0", server.WithToolCapabilities(false))
	...
	s.AddTool(mcp.NewTool("generate_openapi",
		...
		mcp.WithString("config", mcp.Description("JSON config file (default: spector.json in dir, if present)")),
	...
	s.AddTool(mcp.NewTool("mock_request",
		mcp.WithDescription("Call one documented path against spector's in-process mock and return the status, headers and body it answers with. No port is opened. The mock is shape, not state: it answers from the response schema, so repeated calls return the same body."),
		mcp.WithString("dir", mcp.Required(), mcp.Description("directory to scan")),
...
// mcpConfig builds a spector.Config from tool arguments, applying the same
// spector.json defaulting the CLI does. No flags are ever "passed" here, so an
...
func mcpConfig(req mcp.CallToolRequest) (spector.Config, error) {
	dir, err := req.RequireString("dir")
	if err != nil {
		return spector.Config{}, err
	}
	cfg := spector.Config{
		Dir:        dir,
	...
	fs := flag.NewFlagSet("spector-mcp", flag.ContinueOnError)
...
func handleGenerateGrpc(_ context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	doc, err := spector.GenerateGrpc(cfg)
	if err != nil {
		return mcp.NewToolResultError(err.Error()), nil
	}
	return jsonResult(doc)
...
func handleGenerateGraphql(_ context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	doc, err := spector.GenerateGraphql(cfg)
	if err != nil {
		return mcp.NewToolResultError(err.Error()), nil
	}
	return jsonResult(doc)
...
func handleGenerateSDK(_ context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	files, err := spector.GenerateSDK(cfg, spector.SDKOptions{
		Lang:    lang,
...
func handleMockRequest(_ context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	spector.MockHandler(doc, spector.MockOptions{}).ServeHTTP(rec, httpReq)

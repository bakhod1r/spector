...
// returns.
func runProxy(doc *spector.Document, cfg proxyConfig, stdout, stderr io.Writer) int {
	fail := func(err error) int {
		fmt.Fprintln(stderr, "spector:", err)
		return 1
	}

	opts := spector.ProxyOptions{
		Target: cfg.target,
	...
	if cfg.record != "" {
		...
		fmt.Fprintf(stderr, "spector: recording traffic to %s — this file will contain request and response bodies.\n", cfg.record)
		if cfg.recordRaw {
			fmt.Fprintln(stderr, "spector: WARNING: -proxy-record-raw is on, so credentials and sensitive fields are NOT redacted. Do not commit this file.")
		} else {
			fmt.Fprintln(stderr, "spector: credential headers and sensitive-looking fields are redacted, but bodies may still hold personal data. Do not commit this file.")
		}
	}
	...
	p, err := spector.NewProxy(doc, opts)
	if err != nil {
		return fail(err)
	}
	...
	fmt.Fprintf(stderr, "spector: proxying %s -> %s (%d documented paths)\n", cfg.addr, cfg.target, len(doc.Paths))
	fmt.Fprintln(stderr, "spector: forwarding everything; reporting only where traffic disagrees with the document. Ctrl-C to stop.")
	...
	if cfg.report != "" {
		...
		fmt.Fprintf(stderr, "spector: wrote drift report to %s\n", cfg.report)
	}

	if recorder != nil {
		fmt.Fprintf(stderr, "spector: recorded %d exchange(s) to %s\n", recorder.Count(), cfg.record)
	}

	if learner != nil {
		if learner.Empty() {
			fmt.Fprintln(stderr, "spector: nothing to learn — every request matched a documented endpoint")
		} else {
			...
			if cfg.merge {
				out = spector.MergeObserved(doc, out)
			...
			fmt.Fprintf(stderr, "spector: wrote %s to %s (review before merging)\n", what, cfg.learn)
		}
	}

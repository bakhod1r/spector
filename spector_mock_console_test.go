...
// The console's Mock button sends requests to {base}/mock/… . That route is
// mounted whatever Config.Mock says — a switch in the page that only worked
// after a restart with -serve-mock would be a switch that usually does
// nothing — and it must not disturb the catch-all behaviour around it.
func TestConsoleMockRouteWithoutMockMode(t *testing.T) {
	h := Handler(Config{Dir: "examples/shop", Title: "Shop", Version: "1.0.0"})

	for _, path := range []string{"/docs/mock/api/v1/categories", "/mock/api/v1/categories"} {
		rec := httptest.NewRecorder()
		...
		if strings.Contains(rec.Body.String(), "<html") {
			t.Errorf("%s: returned the console HTML, want mock JSON", path)
		}
	}

	// The documented path itself is still the console's catch-all, exactly as
	// it was before the route existed.
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/v1/categories", nil))
	if ct := rec.Header().Get("Content-Type"); !strings.HasPrefix(ct, "text/html") {
		t.Errorf("documented path Content-Type = %q, want text/html (unchanged)", ct)
	}

	// A path the document does not describe is a 404 from the mock, not the
	// console page: inside the mock route the mock answers everything.
	rec = httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/docs/mock/nope", nil))
	if rec.Code != http.StatusNotFound {
		t.Errorf("undocumented mock path: got status %d, want 404", rec.Code)
	}
}

...
func (c *userServiceClient) CountUsers(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[GetUserRequest, CountUsersResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[1], UserService_CountUsers_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[GetUserRequest, CountUsersResponse]{ClientStream: stream}
	return x, nil
}
...
func _UserService_CountUsers_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UserServiceServer).CountUsers(&grpc.GenericServerStream[GetUserRequest, CountUsersResponse]{ServerStream: stream})
}
...
func (UnimplementedProductServiceServer) WatchProducts(*ListProductsRequest, grpc.ServerStreamingServer[Product]) error {
	return status.Errorf(codes.Unimplemented, "method WatchProducts not implemented")
}
